...
export default function HistoryItem({ date, isToday }: Props) {
  ...
  const historyEntry = useHistory((s) => s.history.find((h) => h.date === dateKey))
  ...
  const todos = (historyEntry?.todoIds ?? []).map(id => allTodos.find(t => t.id === id)).filter((t): t is NonNullable<typeof t> => !!t)
    ...
    >
      ...
      {todos && todos.length > 0 && (
        <div className="mt-1 flex flex-col gap-1">
          {todos.map((t) => (
            <div key={t.id} className="w-full flex items-center rounded-sm px-2 py-1 text-sm bg-white/80 text-gray-800">
